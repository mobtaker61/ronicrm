        ...
        <div class="space-y-6">
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex space-x-3 rtl:space-x-reverse">
                        <button
                            ...
                            class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors flex items-center space-x-2 rtl:space-x-reverse"
                        >
                            ...
                            <span>{{ t('customers.share_via_whatsapp') }}</span>
                        </button>
                        <a
                            v-if="customer.share_key"
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                    <!-- Basic Information -->
                    <div class="bg-white rounded-lg shadow p-6">
                        <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ t('customers.basic_information') }}</h3>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div v-if="customer.gender">
                                <label class="block text-sm font-medium text-gray-500 mb-1">{{ t('customers.gender') }}</label>
                                <p class="text-gray-900 capitalize">{{ customer.gender }}</p>
                            </div>
                            <div v-if="customer.languages && customer.languages.length">
                                <label class="block text-sm font-medium text-gray-500 mb-1">{{ t('superadmin.languages') }}</label>
                                <p class="text-gray-900">{{ customer.languages.join(', ') }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-500 mb-1">{{ t('sidebar.industries') }}</label>
                                <span
                                ...
                                <span v-else class="text-gray-400">{{ t('common.dash') }}</span>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-500 mb-1">{{ t('common.status') }}</label>
                                <span
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-500 mb-1">{{ t('customers.source') }}</label>
                                <p class="text-gray-900 capitalize">{{ customer.source || t('common.dash') }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-500 mb-1">{{ t('customers.contact_person') }}</label>
                                <p class="text-gray-900">{{ customer.contact_person || t('common.dash') }}</p>
                            </div>
                            <div class="md:col-span-2">
                                <label class="block text-sm font-medium text-gray-500 mb-1">{{ t('customers.address') }}</label>
                                <p class="text-gray-900">{{ customer.address || t('common.dash') }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div v-if="customer.contacts && customer.contacts.length > 0" class="bg-white rounded-lg shadow p-6">
                        <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ t('customers.contact_methods') }}</h3>
                    ...
                    <div v-if="customer.social_media && customer.social_media.length > 0" class="bg-white rounded-lg shadow p-6">
                        <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ t('customers.social_media_websites') }}</h3>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <a
                    ...
                    <!-- Notes Section -->
                    <div class="bg-white rounded-lg shadow p-6">
                        <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ t('customers.notes') }}</h3>
                        
                        <form @submit.prevent="addNote" class="mb-6">
                            <textarea
                                v-model="noteForm.note"
                                rows="3"
                                :placeholder="t('customers.add_note_placeholder')"
                        ...
                        <div class="space-y-4">
                            <div
                                ...
                                :key="note.id"
                                class="border-l-4 border-blue-500 pl-4 py-2 rtl:border-l-0 rtl:border-r-4 rtl:border-r-blue-500 rtl:pl-0 rtl:pr-4"
                            >
                                <div class="flex justify-between items-start">
                                    <div class="flex-1">
                                        <p class="text-gray-900">{{ note.note }}</p>
                                        <p class="text-sm text-gray-500 mt-1">
                                    {{ t('customers.by_user_on').replace(':user', note.user?.name || t('common.dash')).replace(':date', formatDate(note.created_at)) }}
                                        </p>
                                    </div>
                                    <button
                                        @click="deleteNote(note.id)"
                                        class="text-red-600 hover:text-red-900 ltr:ml-4 rtl:mr-4"
                                    >
                                        {{ t('common.delete') }}
                                    </button>
                                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white rounded-lg shadow p-6">
                        <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ t('customers.recent_conversations') }}</h3>
                        <div v-if="recentMessages && recentMessages.length > 0" class="space-y-3 max-h-96 overflow-y-auto">
                            ...
                            >
                                <div class="flex items-start justify-between mb-1">
                                    ...
                                    >
                                        {{ msg.direction === 'incoming' ? t('customers.received') : t('customers.sent') }}
                                    </span>
                                ...
                                <p v-else-if="msg.media_url" class="text-sm text-gray-500 italic">{{ t('customers.image') }}</p>
                            </div>
                        </div>
                        <div v-else class="text-center py-8 text-gray-500">
                            ...
                            <p class="text-sm">{{ t('customers.no_conversations_yet') }}</p>
                        </div>
                    </div>

                    <!-- Campaigns -->
                    <div class="bg-white rounded-lg shadow p-6">
                        <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ t('sidebar.campaigns') }}</h3>
                        ...
                        <div v-else class="text-center py-8 text-gray-500">
                            ...
                            <p class="text-sm">{{ t('dashboard.no_campaigns_yet') }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        >
            <div class="bg-white rounded-lg shadow-xl max-w-md w-full mx-4">
                <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
                    <h3 class="text-lg font-semibold text-gray-900">{{ t('customers.share_via_whatsapp') }}</h3>
                    <button
...
const deleteNote = (noteId) => {
    if (confirm(t('customers.confirm_delete_note'))) {

    ...
    <AppLayout>
        ...
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                >
                    <div class="flex items-center">
                        ...
                        <div class="ltr:ml-4 rtl:mr-4 flex-1 min-w-0">
                            <p class="text-sm font-medium text-gray-500">{{ t('sidebar.customers') }}</p>
                            <p class="text-2xl font-semibold text-gray-900">{{ stats.total_customers }}</p>
                            <p v-if="stats.customers_this_week > 0" class="text-xs text-green-600">+{{ stats.customers_this_week }} {{ t('dashboard.this_week') }}</p>
                        </div>
                    </div>
                </Link>
                ...
                >
                    <div class="flex items-center">
                        ...
                        <div class="ltr:ml-4 rtl:mr-4 flex-1 min-w-0">
                            <p class="text-sm font-medium text-gray-500">{{ t('dashboard.unread_inbox') }}</p>
                ...
                >
                    <div class="flex items-center">
                        ...
                        <div class="ltr:ml-4 rtl:mr-4 flex-1 min-w-0">
                            <p class="text-sm font-medium text-gray-500">{{ t('sidebar.campaigns') }}</p>
                            <p class="text-2xl font-semibold text-gray-900">{{ stats.total_campaigns }}</p>
                            <p v-if="stats.active_campaigns > 0" class="text-xs text-amber-600">{{ stats.active_campaigns }} {{ t('dashboard.active') }}</p>
                        </div>
                    </div>
                </Link>
                ...
                <div class="bg-white rounded-lg shadow p-5 border border-gray-100">
                    <div class="flex items-center">
                        ...
                        <div class="ltr:ml-4 rtl:mr-4 flex-1 min-w-0">
                            <p class="text-sm font-medium text-gray-500">{{ t('sidebar.templates') }}</p>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
                <Link
                    :href="dashboardChannelHref('telegram', stats.telegram_connected)"
                ...
                >
                    <div class="flex items-center">
                        ...
                        <div class="ltr:ml-3 rtl:mr-3">
                            <p class="font-medium text-gray-900">{{ t('sidebar.telegram') }}</p>
                            <p class="text-sm" :class="stats.telegram_connected ? 'text-green-600' : 'text-gray-500'">
                                {{ stats.telegram_connected ? t('dashboard.connected') : t('dashboard.not_connected') }}
                            </p>
                        </div>
                    </div>
                ...
                <Link
                    :href="dashboardChannelHref('whatsapp', stats.whatsapp_connected)"
                ...
                >
                    <div class="flex items-center">
                        <div
                            ...
                            :class="stats.whatsapp_connected ? 'bg-green-100' : 'bg-gray-100'"
                        >
                            <svg class="w-5 h-5" :class="stats.whatsapp_connected ? 'text-green-600' : 'text-gray-400'" fill="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="ltr:ml-3 rtl:mr-3">
                            ...
                            <p class="text-sm" :class="stats.whatsapp_connected ? 'text-green-600' : 'text-gray-500'">
                                {{ stats.whatsapp_connected ? t('dashboard.connected') : t('dashboard.not_connected') }}
                            </p>
                        </div>
                    </div>
                ...
                <Link
                    :href="dashboardChannelHref('instagram', stats.instagram_connected)"
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="ltr:ml-3 rtl:mr-3">
                            ...
                            <p class="text-sm" :class="stats.instagram_connected ? 'text-green-600' : 'text-gray-500'">
                                {{ stats.instagram_connected ? t('dashboard.connected') : t('dashboard.not_connected') }}
                            </p>
                        </div>
                    </div>
                ...
                <Link
                    :href="dashboardChannelHref('tiktok', stats.tiktok_connected)"
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="bg-white rounded-lg shadow p-6">
                    <div class="flex justify-between items-center mb-4">
                        <h3 class="text-lg font-semibold text-gray-900">{{ t('dashboard.recent_inbox') }}</h3>
                    ...
                    <div class="space-y-2">
                        ...
                        >
                            ...
                            <div
                                ...
                                :class="{
                                    'bg-blue-100': msg.channel === 'telegram' && !msg.avatar,
                                    'bg-green-100': msg.channel === 'whatsapp' && !msg.avatar,
                                    'bg-pink-100': msg.channel === 'instagram' && !msg.avatar,
                                }"
                            >
                                <img
                                    v-if="msg.avatar"
                                ...
                                />
                                <svg
                                    v-else-if="msg.channel === 'telegram'"
                                ...
                                <svg
                                    v-else-if="msg.channel === 'whatsapp'"
                                ...
                                <svg
                                    v-else-if="msg.channel === 'instagram'"
                                    class="w-5 h-5 text-pink-600"
                                    fill="currentColor"
                                    viewBox="0 0 24 24"
                                >
                                    <path d="M12 0C8.74 0 8.333.015 7.053.072 5.775.132 4.905.333 4.14.63c-.789.306-1.459.717-2.126 1.384S.935 3.35.63 4.14C.333 4.905.131 5.775.072 7.053.012 8.333 0 8.74 0 12s.015 3.667.072 4.947c.06 1.277.261 2.148.558 2.913.306.788.717 1.459 1.384 2.126.667.666 1.336 1.079 2.126 1.384.766.296 1.636.499 2.913.558C8.333 23.988 8.74 24 12 24s3.667-.015 4.947-.072c1.277-.06 2.148-.262 2.913-.558.788-.306 1.459-.718 2.126-1.384.666-.667 1.079-1.335 1.384-2.126.296-.765.499-1.636.558-2.913.06-1.28.072-1.687.072-4.947s-.015-3.667-.072-4.947c-.06-1.277-.262-2.149-.558-2.913-.306-.789-.718-1.459-1.384-2.126C21.319 1.347 20.651.935 19.86.63c-.765-.297-1.636-.499-2.913-.558C15.667.012 15.26 0 12 0zm0 2.16c3.203 0 3.585.016 4.85.071 1.17.055 1.805.249 2.227.415.562.217.96.477 1.382.896.419.42.679.82.896 1.381.164.422.36 1.057.413 2.227.057 1.266.07 1.646.07 4.85s-.015 3.585-.074 4.85c-.061 1.17-.256 1.805-.421 2.227-.224.562-.479.96-.899 1.382-.419.419-.824.679-1.38.896-.42.164-1.065.36-2.235.413-1.274.057-1.649.07-4.859.07-3.211 0-3.586-.015-4.859-.074-1.171-.061-1.816-.256-2.236-.421-.569-.224-.96-.479-1.379-.899-.421-.419-.69-.824-.9-1.38-.165-.42-.359-1.065-.42-2.235-.045-1.26-.061-1.649-.061-4.844 0-3.196.016-3.586.061-4.861.061-1.17.255-1.814.42-2.234.21-.57.479-.96.9-1.381.419-.419.81-.689 1.379-.898.42-.166 1.051-.361 2.221-.421 1.275-.045 1.65-.06 4.859-.06l.045.003zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16c-2.21 0-4-1.79-4-4s1.79-4 4-4 4 1.79 4 4-1.79 4-4 4zm7.846-10.405a1.441 1.441 0 10-2.881 0 1.44 1.44 0 002.881 0z"/>
                                </svg>
                            </div>
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-medium text-gray-900 truncate">{{ msg.name || msg.from }}</p>
                                <p class="text-sm text-gray-600 truncate">{{ msg.message || t('dashboard.media_placeholder') }}</p>
                            </div>
                            <div class="flex-shrink-0 flex items-center gap-1">
                                <span v-if="msg.unread" class="w-2 h-2 bg-blue-500 rounded-full" :title="t('dashboard.unread')"></span>
                                <span class="text-xs text-gray-500">{{ formatTime(msg.created_at) }}</span>
                            </div>
                        </div>
                        <p v-if="recentInbox.length === 0" class="text-sm text-gray-500 text-center py-8">{{ t('dashboard.no_recent_messages') }}</p>
                    </div>
                </div>

                <!-- Customers by Status -->
                <div class="bg-white rounded-lg shadow p-6">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ t('dashboard.customers_by_status') }}</h3>
                    <div class="space-y-3">
                        <div
                        ...
                        >
                            <span class="text-sm font-medium text-gray-700 capitalize">{{ status || t('dashboard.unknown') }}</span>
                        ...
                        <p v-if="Object.keys(customersByStatus).length === 0" class="text-sm text-gray-500 py-4">{{ t('dashboard.no_customers_yet') }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div v-if="Object.keys(customersBySource).length > 0" class="bg-white rounded-lg shadow p-6">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ t('dashboard.customers_by_source') }}</h3>
                    <div class="space-y-2">
                        <div
                ...
                <!-- Top Industries -->
                <div class="bg-white rounded-lg shadow p-6">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ t('dashboard.top_industries') }}</h3>
                    <div class="space-y-2">
                        <div
                        ...
                        <p v-if="customersByIndustry.length === 0" class="text-sm text-gray-500 py-4">{{ t('dashboard.no_industries_assigned') }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow p-6">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-semibold text-gray-900">{{ t('dashboard.recent_campaigns') }}</h3>
                ...
                <div class="space-y-3">
                    ...
                    <p v-if="recentCampaigns.length === 0" class="text-sm text-gray-500 text-center py-8">{{ t('dashboard.no_campaigns_yet') }}</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                >
                    ...
                    <span class="text-sm text-gray-600 mt-1">{{ t('dashboard.telegram_groups') }}</span>
                </Link>
                <Link
                ...
                >
                    ...
                    <span class="text-sm text-gray-600 mt-1">{{ t('sidebar.templates') }}</span>
                </Link>
                <Link
                ...
                >
                    <span class="text-2xl font-bold text-gray-900">{{ t('dashboard.crawl') }}</span>
                    <span class="text-sm text-gray-600 mt-1">{{ t('dashboard.telegram_groups') }}</span>
                </Link>
                <Link
                ...
                >
                    <span class="text-2xl font-bold text-gray-900">{{ t('dashboard.scrap') }}</span>
                    <span class="text-sm text-gray-600 mt-1">{{ t('dashboard.web_tasks') }}</span>
                </Link>
            </div>
        </div>
    </AppLayout>
...
/**
 * Inbox when the channel is connected; otherwise settings tab for that integration.
 */
function dashboardChannelHref(channel, connected) {
    const r = typeof window !== 'undefined' ? window.route : null;
    if (!r) {
        return '/inbox';
    }
    if (connected) {
        return r('inbox.index', { channel });
    }
    ...
    const tab = settingsTabByChannel[channel];
    if (!tab) {
        return r('settings.index');
    }

    return r('settings.index', { tab });
}

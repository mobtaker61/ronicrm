<template>
    <div class="min-h-screen bg-gray-50 flex flex-col">
        <header class="bg-white border-b border-gray-200 sticky top-0 z-40">
            <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex items-center justify-between h-16">
                    <Link :href="route('front.welcome')" class="flex items-center gap-3">
                        <div class="bg-blue-600 rounded-lg p-2">
                            <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                            </svg>
                        </div>
                        <span class="text-lg font-bold text-gray-900">{{ appName }}</span>
                    </Link>
                    <nav class="hidden sm:flex items-center gap-6">
                        <Link
                            :href="route('front.welcome')"
                            class="text-sm font-medium text-gray-600 hover:text-blue-600 transition-colors"
                            :class="{ 'text-blue-600': $page.url === '/' }"
                        >
                            Home
                        </Link>
                        <Link
                            :href="route('front.privacy')"
                            class="text-sm font-medium text-gray-600 hover:text-blue-600 transition-colors"
                            :class="{ 'text-blue-600': $page.url.startsWith('/privacy') }"
                        >
                            Privacy Policy
                        </Link>
                        <Link
                            :href="route('front.terms')"
                            class="text-sm font-medium text-gray-600 hover:text-blue-600 transition-colors"
                            :class="{ 'text-blue-600': $page.url.startsWith('/terms') }"
                        >
                            Terms & Conditions
                        </Link>
                        <Link
                            v-if="$page.props.auth?.user"
                            :href="route('dashboard')"
                            class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors"
                        >
                            Dashboard
                        </Link>
                        <Link
                            v-else
                            :href="route('login')"
                            class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors"
                        >
                            Log in
                        </Link>
                    </nav>
                    <!-- Mobile menu button -->
                    <button
                        type="button"
                        class="sm:hidden p-2 rounded-lg text-gray-600 hover:bg-gray-100"
                        @click="mobileMenuOpen = !mobileMenuOpen"
                    >
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path v-if="!mobileMenuOpen" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16" />
                            <path v-else stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                        </svg>
                    </button>
                </div>
                <!-- Mobile menu -->
                <div v-if="mobileMenuOpen" class="sm:hidden py-4 border-t border-gray-200 space-y-2">
                    <Link :href="route('front.welcome')" class="block py-2 text-gray-600 hover:text-blue-600" @click="mobileMenuOpen = false">Home</Link>
                    <Link :href="route('front.privacy')" class="block py-2 text-gray-600 hover:text-blue-600" @click="mobileMenuOpen = false">Privacy Policy</Link>
                    <Link :href="route('front.terms')" class="block py-2 text-gray-600 hover:text-blue-600" @click="mobileMenuOpen = false">Terms & Conditions</Link>
                    <Link
                        v-if="$page.props.auth?.user"
                        :href="route('dashboard')"
                        class="block py-2 font-medium text-blue-600"
                        @click="mobileMenuOpen = false"
                    >
                        Dashboard
                    </Link>
                    <Link
                        v-else
                        :href="route('login')"
                        class="block py-2 font-medium text-blue-600"
                        @click="mobileMenuOpen = false"
                    >
                        Log in
                    </Link>
                </div>
            </div>
        </header>

        <main class="flex-1">
            <slot />
        </main>

        <footer class="bg-white border-t border-gray-200 mt-auto">
            <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div class="flex items-center gap-2">
                        <span class="text-sm font-semibold text-gray-700">{{ appName }}</span>
                    </div>
                    <nav class="flex flex-wrap items-center justify-center gap-6 text-sm text-gray-500">
                        <Link :href="route('front.welcome')" class="hover:text-blue-600 transition-colors">Home</Link>
                        <Link :href="route('front.privacy')" class="hover:text-blue-600 transition-colors">Privacy Policy</Link>
                        <Link :href="route('front.terms')" class="hover:text-blue-600 transition-colors">Terms & Conditions</Link>
                        <Link v-if="!$page.props.auth?.user" :href="route('login')" class="hover:text-blue-600 transition-colors">Log in</Link>
                    </nav>
                </div>
                <p class="mt-6 text-center sm:text-left text-xs text-gray-400">
                    © {{ new Date().getFullYear() }} {{ appName }}. All rights reserved.
                </p>
            </div>
        </footer>
    </div>
</template>

<script setup>
import { Link } from '@inertiajs/vue3';
import { ref } from 'vue';

defineProps({
    appName: { type: String, default: 'RoniCRM' },
});

const mobileMenuOpen = ref(false);
</script>
